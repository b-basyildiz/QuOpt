...
def fidelity_ml(M,input_gate,t,N_iter,rseed,H0,level,subspace):
    ...
    Subspace functionality:
        if Subspace == 0: then just classic qutrit optimizaiton 
        if Subspace == 1: then qutrit optimization but with only qutrit drives
        if Subspace == 2: then qutrit optimization with just y drive 
    """
